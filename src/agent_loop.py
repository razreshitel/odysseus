...
| 1 | 2 |
```
Export to a real Office file — `format:` is docx (Word), xlsx (Excel), or pptx (PowerPoint). ONLY use when the user explicitly asks for one of those formats; otherwise write markdown with create_document. You provide markdown (headings, tables, bullets, code); it renders the binary file and reports the path. For xlsx, markdown tables become sheets; for pptx, `#` headings become slides.""",
